        ...
        if form.is_valid():
            ...
            if user.is_active: ## se usuario for ativado a gente muda o status para True e envia e-mail
                ...
                send_mail( # Envia email para usuario
                    ...
                    settings.os.getenv('DEFAULT_FROM_EMAIL'), # De (em produção usar o e-mail que está no settings)
                    [usuario.email], # para
                    fail_silently=False,
                )
...
def lista_usuarios(request):
    ...
    # Filtra usuários com base no termo de busca, caso fornecido
    if query:
        lista_usuarios = MyUser.objects.select_related('perfil').filter(
            is_superuser=False
        ).filter(
            Q(first_name__icontains=query) | Q(last_name__icontains=query) | Q(email__icontains=query)
        ).order_by('id')
    else:
        lista_usuarios = MyUser.objects.select_related('perfil').filter(is_superuser=False).order_by('id')
    ...
    return render(request, 'lista-usuarios.html', {'page_obj': page_obj, 'query': query})
